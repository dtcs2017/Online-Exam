							...
							<table class="table-striped table">
								...
								<tbody>
									<c:forEach items="${examList }" var="item">
										<tr>
											<td>${item.examName }</td>
											<td><span class="span-info question-number"><fmt:formatDate value="${item.startTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></td>
											<td><span class="span-success question-number-2"><fmt:formatDate value="${item.endTime}" pattern="yyyy-MM-dd HH:mm:ss"/></span></td>
											<td>
											<a href="examing/${item.id }" class="btn btn-success btn-sm join-practice-btn">参加考试</a>		
											</td>
										</tr>
									</c:forEach>
								</tbody>
