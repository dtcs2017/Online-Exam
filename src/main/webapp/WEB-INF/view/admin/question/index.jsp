	...
	<body>
		...
		<div class="admin">
			<form method="post">
				<div class="panel admin-panel">
					...
					<table>
					<tr>
					<td class="my_td">试题名称:<input type="text" name="name" class="my_input">  </td>
					<td class="my_td">试题类型:<select class=" id="questiontypeId" name="questiontypeId" data-validate="required:请选择,length#>=1:至少选择1项">
                            <c:forEach items="${questionTypeList}" var="x">
                            <option value="${x.id}">${x.type}</option>
                         </c:forEach>
                            </select>  </td>
                    <td class="my_td">试题分值:<input type="text" name="points" class="my_input">  </td>
					...
					<table class="table table-hover">
						...
						<tr>
							...
							<td><a class="button border-blue button-little" href="question/update?id=${x.id}">修改</a> <a class="button border-yellow button-little" style="cursor:pointer;" onclick="doDelete(${x.id})">删除</a></td>
						</tr>
						</c:forEach>
					</table>
					...
					<li><a href="admin/question/index?pageNum=${pageBean.thisPageNumber+1}">下一页</a></li>
					<li><a href="admin/question/index?pageNum=${pageBean.lastPageNumber}">尾页</a></li>
					</c:if>
					<c:if test="${pageBean.thisPageNumber>1 && (pageBean.thisPageNumber<pageBean.lastPageNumber)}">
					<li><a href="admin/question/index?pageNum=1">首页</a></li>
					<li><a href="admin/question/index?pageNum=${pageBean.thisPageNumber-1}">上一页</a></li>
					<li><a href="admin/question/index?pageNum=${pageBean.thisPageNumber+1}">下一页</a></li>
					<li><a href="admin/question/index?pageNumr=${pageBean.lastPageNumber}">尾页</a></li>
					</c:if>			
		...
		<div class="hidden">
			...
			function doDelete(id1){
				if(confirm('确认删除?')){
							$.ajax({
						        url: "admin/question/doDelete.action",
						        type: "post",
						        data: {id:id1},
